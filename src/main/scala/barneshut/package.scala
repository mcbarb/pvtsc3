...
package object barneshut {
  ...
  extends Quad {
    val mass : Float = (for (b <- bodies) yield b.mass).sum
    val massX : Float = (for (b <- bodies) yield b.mass * b.x).sum / mass
    val massY : Float = (for (b <- bodies) yield b.mass * b.y).sum / mass
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Leaf(_, _, _, bodies) => {
          ...
          for (b <- bodies) yield addForce(b.mass,b.x,b.y)
        }

...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var m = points.groupBy(p => findClosest(p, means))
    for (mean <- means) yield { m.get(mean) match {
      case None => m += (mean -> GenSeq())
      case Some(e) => {} } }
    m
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (for (i <- 0 until oldMeans.length) yield { newMeans(i).squareDistance(oldMeans(i)) <= eta }).reduce(_ && _)
  }
